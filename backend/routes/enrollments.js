...
// GET /api/enrollments — Get All Enrollments (with student & course names)
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ success: false, message: 'Student already enrolled in this course' });
        res.status(500).json({ success: false, message: err.message });
    }
});
